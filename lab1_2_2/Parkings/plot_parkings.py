...
if __name__=='__main__':
    ...
    for city in ['to','ny','ma']:
        ...
        if city == 'to':
            pyplot.title('CDF parkings in Torino in September 2017')
            fig.savefig(folder + '/'+city+'_parking_ng.png')
        elif city == 'ny':
            pyplot.title('CDF parkings in NYC in September 2017')
            fig.savefig(folder + '/'+city+'_parking_ng.png')
        elif city == 'ma':
            pyplot.title('CDF parkings in Madrid in September 2017')
            fig.savefig(folder + '/'+city+'_parking_ng.png')
    
    fig = pyplot.figure(i,figsize = (20,10))
    pyplot.title('CDF of Parkings in the three cities')
    matplotlib.rcParams.update({'font.size': 35})

    for city in ['to','ny','ma']:
        pyplot.plot(cumulative[city],label=city,linewidth=3.5)
        pyplot.legend(prop={'size':20})
        fig.savefig(folder + '/total_parking_ng.png')
